...
final class NetworkingClient {
    ...
    @discardableResult
    func task(request: URLRequest, completion: @escaping (Swift.Result<Data, Error>) -> Void) -> URLSessionDataTask {
        let task = session.dataTask(with: request) { data, _, error in
            if let error {
                completion(Swift.Result<Data, Error>.failure(error))
                return
            }
            if let data {
                completion(Swift.Result<Data, Error>.success(data))
            }
        }
        task.resume()
        return task
