...
open class DPPickerManager: NSObject, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @objc open var timeZone: TimeZone? = TimeZone.current
    ...
    @objc open func showPicker(title: String?, view: UIView, completion:DPPickerCompletion?) {
        ...
        let image = UIImage(named: "ic_close_gray")?.withRenderingMode(.alwaysTemplate)
        ...
        alertView.view.tintColor = .white
...
internal extension UIViewController {
    ...
    static func topViewController1(from viewController: UIViewController? = UIViewController.root) -> UIViewController? {
        if let tabBarViewController = viewController as? UITabBarController {
            return topViewController1(from: tabBarViewController.selectedViewController)
        } else if let navigationController = viewController as? UINavigationController {
            return topViewController1(from: navigationController.visibleViewController)
        } else if let presentedViewController = viewController?.presentedViewController {
            return topViewController1(from: presentedViewController)
        } else {
